...
class Camera:
    ...
    CROP_Y_START, CROP_Y_END, CROP_X_START, CROP_X_END = 169, 395, 215, 432

    def __init__(
        self, resolution=(640, 480), channels=3, address=0, white_balance=True
    ):
    ...
    def get_image(self):
        ...
        if not ret:
            ...
            return None
        return frame[
            self.CROP_Y_START : self.CROP_Y_END, self.CROP_X_START : self.CROP_X_END
        ]
    ...
    def process_image(self, frame):
        ...
        _, threshold = cv2.threshold(
            grayimg, self.THRESHOLD_VALUE, self.MAX_THRESHOLD_VALUE, cv2.THRESH_BINARY
        )
        ...
        self.calibration_marker = (
            None  # Variable to hold the white square for calibration
        )
        ...
        for contour in contours:
            ...
            if M["m00"] != 0.0:  # Avoid division by zero
                ...
                if shape_name == "Square" and b > 100 and g > 120 and r > 120:
                    self.calibration_marker = {
                ...
                cv2.putText(
                    frame,
                    f"({x}, {y})",
                    (x + 2, y + 5),  # Position the coordinates just below the shape
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (int(b), int(g), int(r)),
                    2,
        ...
        return (
            ...
            detected_shapesdata,
            self.calibration_marker,
        )  # Return the processed image, detected shapes, and calibration marker
    ...
    def run(self):
        while True:
            ...
            if captured_image is not None:
                processed_image, detected_shapesdata, self.calibration_marker = (
